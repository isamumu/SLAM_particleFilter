...
def exact_integration(pose_prev, tau, w, v):
    arg = tau*np.array([[0, -1*w, v], [w, 0, 0], [0, 0, 0]])
    return np.matmul(pose_prev, scipy.linalg.expm(arg))
...
def main():
    ...
    tfm = np.array([[1, 0, 0], [0, 1, 0.8323], [0, 0, 1]]) # no change in rotation, but shift in y
    scale_factor = 0.15
    ...
    prev_lid_idx = 0
    R_prev = np.array([[1, 0, 0], [0, 1, 0], [0, 0, 1]])
    ...
    for t in range(1, len(encoder_new_stamps)):
        ...
        w_T_b = exact_integration(R_prev, tau_imu, avg_w, v)
