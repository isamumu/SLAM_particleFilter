...
def main():
    ...
    xPlot = [[] for i in range(N)]
    yPlot = [[] for i in range(N)]
    thetaPlot = [[] for i in range(N)]
    for i in range(N):
        ...
        for t in range(1, len(encoder_new_stamps)):
            ...
            u = np.array([v, avg_w])
            state = predict(prev_state, u+e, tau_imu)
            ...
            t_prev_imu = imu_new_stamps[idx]
            thetaPlot[i].append(state[2])
            xPlot[i].append(state[0])
            yPlot[i].append(state[1])
