...
def main():
    ...
    width = int(3*min_width)
    height = int(3*min_height)
    ...
    for i in range(len(new_lidar_ranges)):
        ...
        xp = int(coord[0]/scale_factor) # scale the meters into box values
        yp = int(coord[1]/scale_factor)

        if findGamma(occ_map[y_start+yp][x_start+xp]) > 0.8:
    ...
    for t in range(1, len(encoder_new_stamps), 2):
        tau_enc = encoder_new_stamps[t] - t_prev_enc
        ...
        w_var = 0.001# abs(w * 0.01)
        v_var = 0.05# abs(vel * 0.01)
        for i in range(N):
            prev_state = mews[i]
